...
- **Consensus** — both models first answer independently, in parallel, from the same conversation state; neither sees the other's opening. Then they take turns critiquing, starting with the chosen first critic. Each critique runs in a fresh thread that gets the earlier conversation, the question, a hidden shared state (positions, agreements, open disagreements, missing information, facts to verify, changes of mind) and the opponent's latest argument, so context stays flat instead of growing with the debate. The run stops when two consecutive critiques find no material objection, or at the turn limit. Each critique also sees both openings verbatim and its own last turn; each model may rewrite only its own position in the state, and a malformed state is ignored. The model that did not have the last word then writes a summary: the consensus answer with 2/2 or 1/2 agreement tags, each model's final position in its own words, where they disagreed, what changed their minds, and what could still change the answer. The other model checks the summary and any corrections appear on the card, with the total time. The debate collapses beneath it.
