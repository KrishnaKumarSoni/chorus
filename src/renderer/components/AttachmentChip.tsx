...
export function AttachmentChip({ att, onRemove, compact }: { att: Attachment; onRemove?: () => void; compact?: boolean }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }} transition={quick}
      className="inline-flex items-center gap-2 rounded-[10px] border px-2 py-1 text-[12px]" style={{ borderColor: 'var(--line-strong)', background: 'var(--panel-solid)' }} title={att.extractError ?? att.name}>
      ...
      {onRemove && (
        <button className="btn-ghost rounded-md p-0.5" onClick={onRemove} aria-label={`Remove ${att.name}`}>
